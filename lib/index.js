      ...
      for (let i = 0; i < linkData.length; i++) {
        ...
        for (let j = 0; j < categories.list.length; j++) {
          if (typeof categories.list[0] === 'string') {
            ...
            this.config.new_post_name = `${categories.name}/:title.md`;
          } else {
            const tag = Object.keys(categories.list[j])[0];
            args._[0] = categories.list[j][tag];
            this.config.new_post_name = `${categories.name}/${tag}/:title.md`;
            categories.tag = tag;
          }
          await create(args, this.config, this.log, this.post, categories);
        }
        // await create(args);
      }
    } catch (e) {
    ...
    while (cloneDataArr) {
      ...
      while (beforeUrlArr.length) {
        ...
        if (util.isObject(articleData)) {
          ...
          this.config.new_post_name = cloneData.output + articleData.item.categories.join('/') + '/:title.md';
          await this.post.create(articleData.item);
          pageTotal++
